...
class MotionSensor extends Device {
    ...
    getState = async (msgid = new Date().getTime().toString()) => {
        ...
        const msState = MotionSensorTypes.bUDP_MotionSensor_getState_Schema.safeParse(safeResp.data);
        if (!msState.success)
            return await generateError('700101', msgid, 'MotionSensor.getState');
        if (!msState.success)
