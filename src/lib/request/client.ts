...
export const sendRequest = async <T extends baseBDDP>(bddpPacket: T) => {
  ...
  if (msgBody.method == "error") {
    errorResp = generateError("700102", bddpPacket.msgid!, bddpPacket.method);
  ...
  // TODO: Verify works after update to validated dataResp.
  // if (isValidDataResp.success) {
  //     return { success: true, data: { ...dataResp } };
  // }
  if (isValidDataResp.success) {
    return { success: true, data: { ...isValidDataResp.data } };
  }
